...
func (a *Client) ListActivityFeed(params *ListActivityFeedParams, authInfo runtime.ClientAuthInfoWriter) (*ListActivityFeedOK, error) {
	...
	if params == nil {
		params = NewListActivityFeedParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "listActivityFeed",
		Method:             "GET",
		PathPattern:        "/activityFeedEvents",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &ListActivityFeedReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*ListActivityFeedOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for listActivityFeed: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
