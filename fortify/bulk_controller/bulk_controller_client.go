...
func (a *Client) PostBulk(params *PostBulkParams, authInfo runtime.ClientAuthInfoWriter) (*PostBulkOK, error) {
	...
	result, err := a.transport.Submit(&runtime.ClientOperation{
		...
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &PostBulkReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*PostBulkOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for postBulk: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
