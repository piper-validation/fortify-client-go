...
func (a *Client) GetCloudSystemPollStatus(params *GetCloudSystemPollStatusParams, authInfo runtime.ClientAuthInfoWriter) (*GetCloudSystemPollStatusOK, error) {
	...
	if params == nil {
		params = NewGetCloudSystemPollStatusParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "getCloudSystemPollStatus",
		Method:             "GET",
		PathPattern:        "/cloudsystem/pollstatus",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetCloudSystemPollStatusReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*GetCloudSystemPollStatusOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getCloudSystemPollStatus: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
