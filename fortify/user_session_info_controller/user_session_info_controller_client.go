...
func (a *Client) PostUserSessionInfo(params *PostUserSessionInfoParams, authInfo runtime.ClientAuthInfoWriter) (*PostUserSessionInfoOK, error) {
	...
	if params == nil {
		params = NewPostUserSessionInfoParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "postUserSessionInfo",
		Method:             "POST",
		PathPattern:        "/userSession/info",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &PostUserSessionInfoReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*PostUserSessionInfoOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for postUserSessionInfo: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
