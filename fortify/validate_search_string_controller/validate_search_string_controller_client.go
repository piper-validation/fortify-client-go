...
func (a *Client) DoValidateSearchString(params *DoValidateSearchStringParams, authInfo runtime.ClientAuthInfoWriter) (*DoValidateSearchStringOK, error) {
	...
	if params == nil {
		params = NewDoValidateSearchStringParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "doValidateSearchString",
		Method:             "POST",
		PathPattern:        "/validateSearchString",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &DoValidateSearchStringReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*DoValidateSearchStringOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for doValidateSearchString: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
